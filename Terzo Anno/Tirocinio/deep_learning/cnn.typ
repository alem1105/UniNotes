...
= Batch Normalization
L'addestramento delle reti neurali presenta sfide complesse legate alla stabilizzazione della dinamica dei gradienti e alla convergenza dell'ottimizzazione. Man mano che una rete accumula strati, le variazioni nei parametri dei primi livelli alterano la distribuzione delle attivazioni inviate ai livelli successivi. Per affronatare questa problematica è stata introdotta la tecnica della _Batch Normalization_. Questa ha permesso di addestrare architetture molto più profonde con tassi di apprendimento elevati e fornendo una convergenza notevolmente più rapida.

== Training Deep Networks
Quando si adotta la discesa del gradiente stostastica su mini-batch per ottimizzare i parametri, l'aggiornamento dei pesi di un layer intermedio influisce sulla distribuzione delle uscite che lo stesso strato trasmette al livello successivo. Se gli strati precedenti modificano continuamente la propria rappresentazione, gli strati successivi devono adattarsi costantemente a un input la cui media e la cui varianza oscillano nel corso dell'addestramento. Tali oscillazioni rallentano l'ottimizzazione e costringono a impiegare passi estremamente ridotti.

La Batch Normalization affronta questa criticità standardizzando le attivazioni intermedie all'interno di ciascun mini-batch. Consideriamo un vettore di attivazioni $x$ generato da un campione all'interno di un mini-batch $B$. La procedura calcola innanzitutto la media campionaria e la varianza campionaria del mini-batch lungo la dimensione del batch.